...
export async function generateMetadata({
  ...
  return { title: dict.studyReports.porta.metaTitle };
}

export default function PortaStudyPage({ params }: { params: { locale: string } }) {
  const { locale } = params;
  const dict = getDictionary(locale);
  const s = dict.studyReports.porta;
      ...
      <div className="mt-12 grid gap-12 lg:grid-cols-2 lg:items-start">
        <div>
          ...
          <div className="mt-6 relative w-full aspect-[4/3] overflow-hidden rounded-2xl border border-line">
            <Image
              src={media.referenceClientImages[1]}
              alt={s.imageCaption}
              fill
